...
public class BigPirateAI : MonoBehaviour
{
    ...
    public void Attack()
    {
        dame.EnemyAttackCircle(AttackRange, attackPos, 3);
    }
    ...
    void Update()
    {
        ...
        {
            if (health.CurrentHealth <= player.GetComponent<PlayerBaseState>().getSlashAttackDame() + 1)
        ...
        if (anim.isAttack || !check.CheckGround(0.1f) || anim.isJumping)
            return;
        if (checkplayer.CheckPlayerInBack(7f) && !isDetectPlayer)
        {
